...
percent_sig_clines_logReg <- linearClineTable_mod %>% 
  dplyr::select(city, pvalLog_Dist, pvalLog_GMIS, pvalLog_hii) %>% 
  pivot_longer(cols = c('pvalLog_Dist', 'pvalLog_GMIS', 'pvalLog_hii'), names_to = 'group', values_to = 'pvalLog') %>% 
...
percent_sig_clines_logReg_byDirection <- linearClineTable_mod %>% 
  dplyr::select(city, pvalLog_Dist, pvalLog_GMIS, betaLog_Dist, betaLog_GMIS, pvalLog_hii, betaLog_hii) %>% 
  pivot_longer(cols = c('pvalLog_Dist', 'pvalLog_GMIS', 'pvalLog_hii'), names_to = 'group_sig', values_to = 'pvalLog') %>% 
  pivot_longer(cols = c('betaLog_Dist', 'betaLog_GMIS', 'betaLog_hii'), names_to = 'group_dir', values_to = 'betaLog') %>% 
  mutate(sig = case_when(pvalLog < 0.05 ~ 'Yes',
                         TRUE ~ 'No'),
  ...
  filter((group_sig  == 'pvalLog_Dist' & group_dir == 'betaLog_Dist') |
          ...
          (group_sig  == 'pvalLog_hii' & group_dir == 'betaLog_hii'))
